...
拡張の問題で通常起動できない場合は `axum code --safe` を使うと、Pi 本体だけを `-ne` で起動し、`pi-bar` / `pi-header` / `pi-shortcuts` / `pi-goal` / `pi-response-guard` / `pi-guard` / `@tintinweb/pi-subagents` を読み込みません。
