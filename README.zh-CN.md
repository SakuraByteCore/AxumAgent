...
- `pi-shortcuts`（合并自 pi-plan + pi-clear + /subagent）
- `@narumitw/pi-goal`
- `pi-response-guard`
- `pi-guard`
...
若扩展问题导致无法正常启动，可使用 `axum code --safe`，仅以 `-ne` 启动 Pi 本体，不加载 `pi-bar` / `pi-header` / `pi-shortcuts` / `pi-goal` / `pi-response-guard` / `pi-guard` / `@tintinweb/pi-subagents` / `pi-workflow` 。
