import { abortIf, rejectUnknownFields, lastNonEmpty, firstNonEmpty } from "../utils.js";
import { HL_BARE_PREFIX_RE } from "./hash.js";
import { parseHashRef, parseText, type Anchor } from "./parse.js";

export type RAnchor = { line: number; hash: string; hashMatched: boolean };
export type HEdit = { content_lines: string[]; hash_range_inclusive: [Anchor, Anchor] };
export type RHEdit = { content_lines: string[]; hash_range_inclusive: [RAnchor, RAnchor] };
export type HTEdit = { content_lines: string[]; hash_range_inclusive: [string, string] };

interface HMismatch { ref: Anchor; kind: "not_found" | "ambiguous"; candidates?: number[] }
...
function resAnchorFromMap(ref: Anchor, hashIndex: Map<string, number[]>): RAnchor | HMismatch {
  ...
  if (hashMatches.length === 1) return { line: hashMatches[0]!, hash: ref.hash, hashMatched: true };
...
export function fmtMismatch(mismatches: HMismatch[], fileLines: string[], fileHashes: string[], filePath?: string): string {
  ...
  if (notFound.length > 0) {
    out.push(`[E_STALE_ANCHOR] ${notFound.length} stale anchor${notFound.length > 1 ? "s" : ""}${filePath ? ` in ${filePath}` : ""}: ${refList}. The file content has changed since those anchors were read. Call read() to get fresh anchors.`);
  }
  if (ambiguous.length > 0) {
    if (out.length > 0) out.push("");
    out.push(`[E_AMBIGUOUS_ANCHOR] ${ambiguous.length} ambiguous anchor${ambiguous.length > 1 ? "s" : ""}${filePath ? ` in ${filePath}` : ""}. Call read() to get fresh anchors.`);
...
function assertItem(edit: Record<string, unknown>, index: number): void {
  ...
  if ("hash_range_inclusive" in edit && !isStrPair(edit.hash_range_inclusive)) {
    throw new Error(`[E_BAD_SHAPE] Edit ${index} field "hash_range_inclusive" must be a pair of anchor strings [start, end].`);
  }
  ...
  if ("content_lines" in edit && !isStrArr(edit.content_lines)) {
    ...
    if (typeof val === "string") {
      try {
        ...
        if (Array.isArray(parsed)) { edit.content_lines = parsed; } else { throw new Error("not array"); }
      } catch {
        throw new Error(`[E_BAD_SHAPE] "content_lines" must be a native JSON array of strings, not a JSON string. Pass it as a proper JSON array.`);
      }
    } else {
  ...
  if (!isStrPair(edit.hash_range_inclusive)) {
    throw new Error(`[E_BAD_SHAPE] Edit ${index} requires an "hash_range_inclusive" pair of anchor strings [start, end].`);
  }
}
...
export function warnUnicodeEsc(edits: HEdit[], warnings: string[]): void {
...
export function assertNoBarePrefix(edits: HEdit[], fileLines: string[], fileHashes: string[]): void {
  ...
  const matchedCount = suspects.filter((s) => fileHashSet.has(s.hash)).length;
...
function checkBoundaryDup(
  ...
  if (!adjacentLine || !replacementEdge || replacementEdge.length === 0 || replacementEdge !== adjacentLine) return null;
  return { kind, survivingLineContent: adjacentLine, survivingLineIndex, replacementLineContent: replacementEdge, editIndex };
}
...
export function valEdits(
  ...
  const tryResolve = (ref: Anchor): RAnchor | undefined => {
    ...
    if ("kind" in result) { mismatches.push(result); return undefined; }
    return result;
  };
  ...
  for (const edit of edits) {
    ...
    if (startResolved.line > endResolved.line) {
      throw new Error(`[E_BAD_OP] Range start line ${startResolved.line} must be <= end line ${endResolved.line} (anchors ${edit.hash_range_inclusive[0].hash} and ${edit.hash_range_inclusive[1].hash}).`);
    }
    ...
    resolved.push({ content_lines: edit.content_lines, hash_range_inclusive: [startResolved, endResolved] });
  }
