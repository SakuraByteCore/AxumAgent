...
type RESpan = { kind: "replace"; index: number; label: string; start: number; end: number; replacement: string };
...
function throwConflict(left: { index: number; label: string }, right: { index: number; label: string }, reason: string): never {
  throw new Error(`[E_EDIT_CONFLICT] Edit ${left.index} (${left.label}) and edit ${right.index} (${right.label}) ${reason}.`);
}
...
function resToSpan(edit: RHEdit, index: number, content: string, lineIndex: LIdx, noopEdits: NEdit[]): RESpan | null {
  ...
  if (originalLines.length === edit.content_lines.length && originalLines.every((line, li) => line === edit.content_lines[li])) {
    noopEdits.push({ editIndex: index, loc: edit.hash_range_inclusive[0].hash, currentContent: originalLines.join("\n") });
    return null;
  }
  const label = descEdit(edit);
  if (edit.content_lines.length > 0) {
    return { kind: "replace", index, label, start: lineStarts[startLine - 1]!, end: lineStarts[endLine - 1]! + fileLines[endLine - 1]!.length, replacement: edit.content_lines.join("\n") };
  }
  if (startLine === 1 && endLine === fileLines.length) {
    return { kind: "replace", index, label, start: 0, end: content.length, replacement: "" };
  }
  if (endLine < fileLines.length) {
    return { kind: "replace", index, label, start: lineStarts[startLine - 1]!, end: lineStarts[endLine]!, replacement: "" };
  }
  return { kind: "replace", index, label, start: Math.max(0, lineStarts[startLine - 1]! - 1), end: lineStarts[endLine - 1]! + fileLines[endLine - 1]!.length, replacement: "" };
}

function assertNoConflict(spans: RESpan[]): void {
  for (let li = 0; li < spans.length; li++) {
    const left = spans[li]!;
    for (let ri = li + 1; ri < spans.length; ri++) {
      const right = spans[ri]!;
...
function resSpans(edits: RHEdit[], content: string, lineIndex: LIdx, noopEdits: NEdit[], signal: AbortSignal | undefined): RESpan[] {
  ...
  return [...resolvedSpans].sort((l, r) => (r.end !== l.end ? r.end - l.end : l.index - r.index));
}
...
export function applyEdits(
  ...
  const { resolved: initialResolved, mismatches, boundaryWarnings } = valEdits(edits, lineIndex.fileLines, fileHashes, warnings, signal);
