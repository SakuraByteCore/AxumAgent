...
fn resolve_server_bin(bin: &str) -> String {
    ...
    if Path::new("target/release/axum-server").exists() {
        return "target/release/axum-server".to_string();
    }
    if Path::new("rs/target/release/axum-server").exists() {
        return "rs/target/release/axum-server".to_string();
    }
    if Path::new("target/debug/axum-server").exists() {
        return "target/debug/axum-server".to_string();
    }
    if Path::new("rs/target/debug/axum-server").exists() {
        return "rs/target/debug/axum-server".to_string();
    }
    bin.to_string()
}
...
async fn ensure_server_bin(
    ...
    if spawn.server_bin != "axum-server" {
        return Ok(resolved_bin.to_string());
    }
    if resolved_bin != "axum-server" {
        return Ok(resolved_bin.to_string());
    }
    if Path::new("rs/Cargo.toml").exists() {
        let _ = build_server().await?;
        let exe = if cfg!(windows) { ".exe" } else { "" };
        let candidate = format!("rs/target/release/axum-server{exe}");
...
async fn build_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    cmd.args(["build", "-q", "-p", "axum-server", "--release"]);
    if Path::new("rs/Cargo.toml").exists() {
        cmd.current_dir("rs");
    }
...
fn default_db_path() -> String {
    ...
    std::env::temp_dir()
        .join(format!("axum-cli-{pid}-{ts}.db"))
        .to_string_lossy()
        .to_string()
}
