...
impl RunManager {
    ...
    pub async fn enqueue(&self, task: String, max_steps: u32) -> Result<RunRequest, String> {
        ...
        tokio::task::spawn_blocking(move || db.insert_run(&run_id_for_db, &task_for_db, "queued", now_ms))
            .await
            .map_err(|_| "db_join_error".to_string())?
            .map_err(|e| format!("db_error:{e}"))?;

        let req = RunRequest { run_id, task, max_steps };
        self.tx.send(req.clone()).await.map_err(|_| "queue_closed".to_string())?;
        Ok(req)
    }

    pub async fn emit(&self, run_id: &str, kind: &str, payload: Value) -> Result<StoredEvent, String> {
        let db = self.db.clone();
        let run_id_s = run_id.to_string();
        let kind_s = kind.to_string();
        let now_ms = now_ms();
        let stored = tokio::task::spawn_blocking(move || db.append_event(&run_id_s, &kind_s, &payload, now_ms))
            .await
            .map_err(|_| "db_join_error".to_string())?
            .map_err(|e| format!("db_error:{e}"))?;
    ...
    pub async fn list_events_since(&self, run_id: &str, from_seq: i64, limit: i64) -> Result<Vec<StoredEvent>, String> {
        let db = self.db.clone();
        let run_id_s = run_id.to_string();
        let events = tokio::task::spawn_blocking(move || db.list_events_since(&run_id_s, from_seq, limit))
            .await
            .map_err(|_| "db_join_error".to_string())?
            .map_err(|e| format!("db_error:{e}"))?;
        Ok(events)
