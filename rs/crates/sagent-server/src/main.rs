...
use sagent_llm::{OpenAiCompatiblePlanner, Planner, PlannerContext};
use sagent_policy::{AllowAllPolicy, Policy};
use sagent_runtime::{RuntimeEvent, RuntimeHooks};
use sagent_tools::{EchoTool, FsTool, ToolRegistry};
use sagent_types::{
...
async fn worker_loop(manager: RunManager, mut rx: tokio::sync::mpsc::Receiver<ManagerRunRequest>) {
    while let Some(req) = rx.recv().await {
        ...
        let result = sagent_runtime::run_agent(&hooks, &task, max_steps, || false).await;
        drop(hooks);
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let db_path = std::env::var("SAGENT_DB_PATH").unwrap_or_else(|_| "data/sagent.db".to_string());
