...
use axum_types::{Action, AgentError, Decision, NotifyLevel, ToolAction};
...
pub fn config_path() -> PathBuf {
    if let Ok(path) = std::env::var("AXUM_CONFIG") {
    ...
    if let Ok(home) = std::env::var("HOME") {
        return PathBuf::from(home).join(".axum").join("config.json");
    }
    PathBuf::from(".axum").join("config.json")
}
...
impl Planner for OpenAiCompatiblePlanner {
    async fn plan(&self, ctx: PlannerContext<'_>) -> Result<Decision, AgentError> {
        ...
        Ok(Decision {
            ...
            usage: parsed.usage.map(|u| axum_types::TokenUsage {
