...
type ActivityStatus = "pending" | "running" | "succeeded" | "failed" | "blocked";
...
interface RuntimeDashboardState {
  ...
  calls: Map<string, { name: string; arguments: Record<string, unknown>; summary: string; intent: string; target: string }>;
}
...
function renderActivityLine(activity: RuntimeActivity): string {
  const mark: Record<ActivityStatus, string> = {
    pending: "○",
    running: "▶",
    succeeded: "✓",
    failed: "✗",
...
function summarizeToolAction(name: string, args: Record<string, unknown>): string {
  ...
  if (name === "lsp_symbols") return `Inspect TypeScript symbols${stringField(args, "query") ? ` for ${stringField(args, "query")}` : ""}`;
...
function summarizeCommandAction(command: string): string {
  ...
  if (program === "ls") return `Inspect ${firstPathLabel(rest) ?? "directory"}`;
  if (program === "find") return `Scan ${firstPathLabel(rest) ?? "workspace"}`;
  if (program === "grep") return `Search ${firstPathLabel(rest.slice(1)) ?? "workspace"}`;
  if (program === "cat" || program === "head" || program === "tail" || program === "sed") return `Inspect ${firstPathLabel(rest) ?? "file"}`;
  if (program === "wc") return `Count ${firstPathLabel(rest) ?? "workspace"}`;
...
function targetForTool(name: string, args: Record<string, unknown>): string {
  if (name === "safe_exec") return renderCommand(args);
  if (name === "read" || name === "precise_edit") return fileForArgs(args);
  if (name === "lsp_symbols") return stringField(args, "query") ?? "TypeScript symbols";
  return name;
}

function intentForTool(name: string, args: Record<string, unknown>, summary: string): string {
  const intent = stringField(args, "intent") ?? stringField(args, "reason");
  if (intent) return clipDetail(intent, 120);
  if (name === "read") return "Gather source evidence before deciding on a change.";
  if (name === "precise_edit") return "Apply a bounded edit after locating the exact target.";
  if (name === "lsp_symbols") return "Map code structure before editing or explaining behavior.";
  if (name === "safe_exec") return intentForCommand(renderCommand(args));
  return `Use ${summary} to move the task forward.`;
}

function intentForCommand(command: string): string {
  const [program, ...rest] = command.split(/\s+/g).filter(Boolean);
  if (program === "npm" && rest.join(" ") === "test") return "Validate the implementation with the full test suite.";
  if (program === "npm" && rest.join(" ") === "run pack:dry") return "Verify the package can be built and packed without publishing.";
  if (program === "git" && rest[0] === "status") return "Confirm workspace cleanliness before reporting or committing.";
  if (program === "git" && rest[0] === "diff") return "Review the exact local changes before validation or commit.";
  if (program === "grep") return "Find the relevant implementation or regression surface.";
  if (["cat", "sed", "head", "tail"].includes(program ?? "")) return "Inspect concrete source context before acting.";
  return "Use an allowlisted command to collect evidence or validate work.";
}

function meaningForToolRequest(name: string, args: Record<string, unknown>): string {
  if (name === "safe_exec") return "The runtime is collecting command evidence, not just printing a raw command.";
  if (name === "read") return "The runtime is inspecting source context before deciding.";
  if (name === "precise_edit") return "The runtime is about to mutate a specific file through a gated edit.";
  if (name === "lsp_symbols") return "The runtime is mapping code structure before making claims.";
  return "The runtime requested a tool step and will interpret the result.";
}

function resultMeaning(name: string, args: Record<string, unknown>, content: string): string {
  if (name === "safe_exec") return commandResultMeaning(renderCommand(args), content);
  if (name === "read") return "The requested file content is now available as evidence for the next step.";
  if (name === "precise_edit") return "The requested file change was applied through the runtime gate.";
  if (name === "lsp_symbols") return "The symbol lookup result can guide the next code-level decision.";
  return "The tool step completed and produced evidence for the next decision.";
}

function commandResultMeaning(command: string, content: string): string {
  const [program, ...rest] = command.split(/\s+/g).filter(Boolean);
  const parsed = parseJsonObject(content);
  const stdout = typeof parsed?.stdout === "string" ? parsed.stdout : "";
  const stderr = typeof parsed?.stderr === "string" ? parsed.stderr : "";
  const output = `${stdout}\n${stderr}`;
  if (program === "npm" && rest.join(" ") === "test") return "Tests passed, covering build, syntax checks, CLI regressions, and TUI snapshots.";
  if (program === "npm" && rest.join(" ") === "run pack:dry") return "Package dry-run passed, so the distributable contents still build.";
  if (program === "git" && rest[0] === "status") return output.trim() ? "Git status found local changes that still need review." : "Git status is clean.";
  if (program === "git" && rest[0] === "diff") return output.trim() ? "The diff shows the exact files and lines changed." : "There is no local diff.";
  return output.trim() ? "The command completed and produced output to interpret." : "The command completed without extra output.";
}

function contextEvidence(payload: Record<string, unknown> | undefined): string {
  if (!payload) return "workspace context captured";
  const cwd = stringField(payload, "cwd") ?? "workspace";
  const tools = Array.isArray(payload.allowedTools) ? payload.allowedTools.filter((tool): tool is string => typeof tool === "string") : [];
  return tools.length > 0 ? `${cwd}; tools: ${tools.join(", ")}` : cwd;
}

function firstPathLabel(args: string[]): string | undefined {
  const value = args.find((arg) => arg && !arg.startsWith("-") && arg !== "|" && arg !== "&&" && arg !== ";");
  if (!value || value === "." || value === "./") return value ? "workspace" : undefined;
...
function evidenceForToolRequest(name: string, args: Record<string, unknown>, summary: string): string {
  ...
  if (name === "read" || name === "precise_edit" || name === "lsp_symbols") return summary;
...
function pushActivity(state: RuntimeDashboardState, activity: RuntimeActivity): void {
  const previous = state.plan[state.plan.length - 1];
  if (previous && previous.action === activity.action && previous.status === activity.status && previous.target === activity.target) return;
  state.plan.push(activity);
}

function finishActivity(state: RuntimeDashboardState, action: string, status: ActivityStatus, evidence: string, meaning: string, next: string): void {
...
function completeLatestActivity(state: RuntimeDashboardState, status: ActivityStatus, evidence: string, meaning: string, next: string): void {
...
function lastLines<T>(lines: T[], max: number): T[] {
