...
This is still an early CLI/provider/runtime slice, not a hardened autonomous agent runtime. The session/event/tool-loop path now exists, and `axum parallel` has a pi-style child-task state model plus merge-review metadata, but managed child-agent execution is not enabled yet. Runtime tool execution is project-sandboxed: `read` and `precise_edit` stay inside the current workspace, while `safe_exec` accepts allowlisted commands such as `npm test` plus guarded workspace inspection commands (`pwd`, `ls`, `find`, `grep`, `cat`, `sed`, `head`, `tail`, `wc`, and read-only `git` subcommands) by splitting simple shell-like command strings into command/args without enabling arbitrary shell operators. TUI runtime state is rendered as an auditable workbench: Activity, Commands, Files, and Blocked sections show what the agent tried, which commands/files were touched, redacted/folded command output summaries, and sandbox blockers. Working status uses human-readable elapsed time (`5m 29s`, `1h 4m`) and includes the latest runtime activity when available. Raw pi-tui owns the live input editor directly, so the old boxed Prompt panel is not rendered above the input line. Repeated identical permission denials fail fast instead of spinning until the tool-iteration limit. The next hardening step is to connect queued/running child tasks to isolated execution, cancellation, failure handling, and merge review without letting the TUI state become the source of truth.
